...
public class MainActivity extends Activity {
	...
	private static final int SETVIEW = 101;
	// string
	...
	private BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (playMode == SINGE) {
				...
				playAndPause.setImageDrawable(getResources().getDrawable(
						R.drawable.img_playback_bt_play));
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Log.i("main activity", "request");
		requestWindowFeature(Window.FEATURE_NO_TITLE);
		setContentView(R.layout.splash);
		Log.i("main activity", "set content view compelete");
		new InitDataTask().execute((Void[]) null);
		Log.i("main activity", "InitDataTask");
	}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
			...
			AudioManager manager = (AudioManager) getSystemService(AUDIO_SERVICE);
			manager.getStreamVolume(AudioManager.STREAM_RING);
			manager.adjustVolume(AudioManager.ADJUST_RAISE,
	...
	private void showNotification() {
		...
		notification = new Notification.Builder(this).setAutoCancel(true)
				.setSmallIcon(R.drawable.icon_notification_play)
				.setContentTitle("天籁之音").setContentIntent(contentIntent)
	...
	public String getLastPath() {
		...
		SharedPreferences preference = getSharedPreferences(TIANLAIZHIYIN,
	...
	protected void changePlayMode() {
		...
		if (current == SINGE) {
			...
			setPlayMode(ORDER);
			playMode.setImageDrawable(getResources().getDrawable(
					R.drawable.icon_playmode_normal));
		} else if (current == ORDER) {

			setPlayMode(SINGE);
			playMode.setImageDrawable(getResources().getDrawable(
					R.drawable.icon_playmode_shuffle));
		}
	}
	...
	private void initViews() {
		// TODO
		Log.i("initView", "set fragment");
		framelayout = (RelativeLayout) LayoutInflater.from(
				getApplicationContext()).inflate(R.layout.framelayout, null);

		Log.i("initView", "set scrollview");
		scrollView = (ScrollableViewGroup) framelayout
				.findViewById(R.id.ViewFlipper);
		scrollView
				.setOnCurrentViewChangedListener(mOnCurrentViewChangedListener);

		Log.i("initView", "set lyrclayout");
		lyrcLayout = (ViewGroupHook) scrollView.findViewById(R.id.frmLyric);
		lrcShow = (LyricShow) lyrcLayout.findViewById(R.id.LyricShow);

		Log.i("initView", "set playlayout");
		...
		Log.i("initView", "set mainimagebutton");
		...
		lyricImageButton = (ImageButton) framelayout
				.findViewById(R.id.IndLyric);
		volumeImageButton = (ImageButton) framelayout
				.findViewById(R.id.ImgBtnVolume);
		currentMusicTitle = (TextView) framelayout.findViewById(R.id.txtTitle);

		playModeButton = (ImageButton) framelayout
				.findViewById(R.id.IndPlayMode);
		preMusicButton = (ImageButton) framelayout.findViewById(R.id.btnPrev);
		playAndPauseButton = (ImageButton) framelayout
				.findViewById(R.id.btnPlay);
		nextMusicButton = (ImageButton) framelayout.findViewById(R.id.btnNext);
		settingsPauseButton = (ImageButton) framelayout
	...
	private void play() {
		...
		playAndPause.setImageDrawable(getResources().getDrawable(
				R.drawable.img_playback_bt_pause));
		String path = musicList.get(currentPosition).getPath();
		String duration = durationToString(musicList.get(currentPosition)
				.getDuration());
		durationTime.setText(duration);
		currentMusicTitle.setText("当前歌曲:"
				+ musicList.get(currentPosition).getTitle());
		Intent i = new Intent();
		i.setAction(MediaService.ACTION);
	...
	private void initLrc() {
		...
		if (file.isDirectory()) {
			...
			for (int i = 0; i < f.length; i++) {
				...
				if (f[i].getName().substring(0, f[i].getName().indexOf("."))
						.equals(name + "-" + s)) {
	...
	public void initCurrentPosition() {
		...
		if (getLastPath().trim() != "") {
			...
			if (musicList.size() > 0) {
				...
				durationTime.setText(durationToString(musicList.get(
	...
	class InitDataTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			// singer
			Log.i("do in background", "init music");
			initMusics();
			Log.i("do in background", "init music compelete");
			lycrUtil = new LyrcUtil(MainActivity.this);
			Log.i("do in background", "init view");
			Message msg = new Message();
			msg.what = SETVIEW;
			handler.sendMessage(msg);
			// initViews();
			Log.i("do in background", "init view compelete");
			Intent i = new Intent(MainActivity.this, MediaService.class);
			startService(i);
			Log.i("do in background", "start service ");
			registerReceiver(receiver, new IntentFilter(ONPLAYCOMPLETED));
			Log.i("do in background", "all compelete");
			return null;
		}
